...
pub = rospy.Publisher('master_string_topic', String, queue_size=1)
...
def calculation_and_display_function():
    ...
    global ball_area , ball_cooridinate , is_ball_detected , ball_accuracy
    ...
    if((goal_marker_distance <= min_dist_for_camera_processing and search_count<8 and node_run_first_time==0) or stay_in_affinity==1):#affinity circle
        ...
        if((is_ball_detected==1.0 and ball_accuracy>80 and ball_area>1300) or left_right_searching_operaion==-1):#if ball detected then run only this
            ...
            if((ball_cooridinate>=0 and ball_alignment_already_started_before==0)or (ball_alignment_already_started_before==-1)):#if ball is on right side, then move left side
                ...
                if(ball_cooridinate<0 and ball_cooridinate>-50):
                    print("!! NOW ROVER IS FACING THE BALL !! :) ")#marker is now located
                    time.sleep(1)
                    master_string="00000"
                    pub.publish(master_string)
                    ball_alignment_already_started_before=8
            ...
            elif(ball_alignment_already_started_before==8):
                #print("Rover is facing the ball !!")
                #master_string="11500"
                #pub.publish(master_string)
                """
                if(ball_area>1200):
                    print("ball is now near of the rover")
                    master_string="00000"
                    pub.publish(master_string)
                """
        ...
        if(is_ball_detected==1):
            print("Ball coordinate : "+str(ball_cooridinate))

    elif(is_obstacle_detected!=0 and False and goal_marker_distance>min_dist_for_camera_processing and node_run_first_time==0 or turn_90_already_started_before!=0):#if lidar detects obstacle
...
def callback_ball(ball_data):
    global ball_area , ball_cooridinate , is_ball_detected , ball_accuracy
    ball_area=ball_data.y
    ball_cooridinate=ball_data.x
    is_ball_detected=ball_data.w
    ball_accuracy=ball_data.z
...
def listener():
    ...
    rospy.Subscriber("ball_data_topic", Quaternion , callback_ball)
